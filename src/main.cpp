  ...
  if (FLAGS_rec)
  {
    ...
    if (FLAGS_inference_framework == "ov")
    {
      ...
      if (FLAGS_inference_device == "NPU")
      {
        ...
        std::string model_small_path = rec_model_path + "/inference_240_bs1.xml";
        std::string model_medium_path = rec_model_path + "/inference_480_bs1.xml";
        ...
        if (!file_small.good() || !file_medium.good() || !file_big.good())
        {
          std::cout << "Error: NPU recognition model files not found in '" << rec_model_path << "'" << std::endl;
          std::cout << "Required files: inference_240_bs1.xml, inference_480_bs1.xml, inference_800_bs1.xml" << std::endl;
          exit(1);
        }
      }
      else
