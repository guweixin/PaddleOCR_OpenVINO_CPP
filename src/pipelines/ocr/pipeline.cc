      ...
      for (int l = 0; l < indices.size(); l++) {
        ...
        for (auto &item : sorted_subs_info) {
          ...
          if (device =="npu"){
            ...
            // int huge_width;  // 默认大模型宽度
            int unknown_width;  // 默认大模型宽度
            ...
            if (rec_ptr) {
              ...
              if (npu_sizes.size() >= 1) {
                ...
                // huge_width = npu_sizes[5].second;
                // unknown_width = npu_sizes[6].second;
                unknown_width = npu_sizes[2].second;
                } else {
            ...
            // float large_threshold = static_cast<float>(large_width) / static_cast<float>(target_h);  
            // float huge_threshold = static_cast<float>(huge_width) / static_cast<float>(target_h);  
            int target_w = 0;        
            ...
            //   // selected_model_type = NPURecModelSize::BIG; // model_type = 3
            //   target_w = big_width;
            // } else if (aspect_ratio <= large_threshold) {
            //   // selected_model_type = NPURecModelSize::BIG; // model_type = 3
            //   target_w = large_width;
            // } else if (aspect_ratio <= huge_threshold) {
            //   // selected_model_type = NPURecModelSize::BIG; // model_type = 3
            //   target_w = huge_width;
            } else {
              // selected_model_type = NPURecModelSize::LARGE;  // model_type = 5
              target_w = unknown_width;
            }
